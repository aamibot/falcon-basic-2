...
auth_middleware = FalconAuthMiddleware(
    auth_backend, exempt_routes=["/auth","/register"], exempt_methods=["HEAD", "OPTIONS"]
)

api = falcon.API(middleware=[Marshmallow(),auth_middleware])  # middleware=[auth_middleware] - add to enable jwt authentication
